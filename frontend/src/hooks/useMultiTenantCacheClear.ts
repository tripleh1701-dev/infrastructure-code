...
/**
 * Hook to clear React Query cache when account, enterprise, or product selection changes.
...
export function useMultiTenantCacheClear() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (currentAccountId !== prevAccountId.current ||
        ...
        currentProductId !== prevProductId.current)
    ) {
      // Set transitioning state
      setIsTransitioning(true);
      
      // Invalidate all cached queries when account, enterprise, or product changes
  ...
  }, [selectedAccount?.id, selectedEnterprise?.id, selectedProduct?.id, queryClient]);
