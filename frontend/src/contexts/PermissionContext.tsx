...
export function PermissionProvider({ children }: { children: ReactNode }) {
  ...
  const fetchPermissionsExternal = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        setPermissions(data.permissions.length > 0 ? data.permissions : DEFAULT_ADMIN_PERMISSIONS);
        setCurrentUserRoleId(data.roleId);
        setCurrentUserRoleName(data.roleName);
        setCurrentTechnicalUserId(data.technicalUserId);
      }
    } catch (error) {
      console.error("Error fetching permissions from API:", error);
      setPermissions(DEFAULT_ADMIN_PERMISSIONS);
