...
import { useUpdateGroup, Group, useGroupRoles, RoleWithPermissions, useCheckGroupNameExists } from "@/hooks/useGroups";
...
export function EditGroupDialog({ open, onOpenChange, group }: EditGroupDialogProps) {
  ...
  // Check for duplicate group name within the same account + enterprise combination (excluding current group)
  const { isDuplicate: isNameDuplicate, isChecking: isCheckingName } = useCheckGroupNameExists(
    formData.name,
    selectedAccount?.id,
    selectedEnterprise?.id,
    group?.id // Exclude current group when editing
  );
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Update group settings, scoping, and role assignments
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {currentStep < STEPS.length ? (
                  ...
                  <Button
                    ...
                    disabled={!isComplete || updateGroup.isPending}
