...
interface EnterpriseWithProduct {
  id: string;
  name: string;
...
export function AddEnterpriseForm({ open, onOpenChange, onSuccess }: AddEnterpriseFormProps) {
  const [enterpriseName, setEnterpriseName] = useState("");
  const [nameError, setNameError] = useState("");
  const [existingEnterprises, setExistingEnterprises] = useState<EnterpriseWithProduct[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      if (isExternalApi()) {
        // External: use service abstractions + httpClient for enterprise_products
        ...
        const filteredServices = (servicesRes.data || []).filter(s => s.id !== GLOBAL_ITEM_ID);

        const enterprisesWithProducts: EnterpriseWithProduct[] = (enterprisesRes.data || []).map((enterprise) => {
          const productLink = (enterpriseProductsRes.data || []).find(
            (link) => link.enterprise_id === enterprise.id
          );
          return { ...enterprise, product_id: productLink?.product_id || null };
        });

        setProducts(filteredProducts.map(p => ({ id: p.id, name: p.name, description: p.description })));
        setServices(filteredServices.map(s => ({ id: s.id, name: s.name, description: s.description })));
        setExistingEnterprises(enterprisesWithProducts);
      } else {
        ...
        const [productsRes, servicesRes, enterprisesRes, enterpriseProductsRes] = await Promise.all([
          ...
          supabase.from("enterprise_products").select("enterprise_id, product_id"),
        ]);
  ...
  // Get products already used with the same enterprise name (case-insensitive)
  const getUsedProductsForName = (name: string): string[] => {
    const trimmedName = name.trim().toLowerCase();
    return existingEnterprises
      .filter((enterprise) => enterprise.name.toLowerCase() === trimmedName && enterprise.product_id)
      .map((enterprise) => enterprise.product_id as string);
  };

  const isProductDisabled = (productId: string): boolean => {
    const usedProducts = getUsedProductsForName(enterpriseName);
    return usedProducts.includes(productId);
  };
  ...
  const handleNameChange = (value: string) => {
    ...
    // Reset product selection if the currently selected product becomes disabled for this name
    if (selectedProduct) {
      const usedProducts = getUsedProductsForName(value);
      if (usedProducts.includes(selectedProduct)) {
        setSelectedProduct(null);
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await enterprisesService.create({
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Create enterprise with product & services
              </p>
            </div>
          </div>
        </motion.div>
                    ...
                    {isFetching ? (
                      ...
                      >
                        {products.map((product, index) => {
                          const disabled = isProductDisabled(product.id);
                          ...
                          return (
                            <motion.div
                              ...
                              className={cn(
                                "flex items-center gap-3 p-3 rounded-lg border transition-all bg-background cursor-pointer",
                                disabled
                                  ? "cursor-not-allowed opacity-50 border-muted"
                                  : "",
                                !disabled && isSelected
                                  ? "border-primary bg-primary/5 ring-2 ring-primary/20 shadow-md"
                                  : !disabled && "hover:border-primary/50 hover:shadow-sm"
                              )}
                              onClick={() => !disabled && setSelectedProduct(product.id)}
                              title={disabled ? `Already used with "${enterpriseName.trim()}"` : undefined}
                            >
                              <motion.div
            ...
            <div className="flex items-center justify-between p-6">
              ...
              >
                ...
                >
                  ...
                  {isLoading ? "Creating..." : "Create Enterprise"}
                </Button>
              </motion.div>
            </div>
          </div>
